...
dfSuitTemp <- dfSuitTemp[,-(seq(3,33,2))]
names(dfSuitTemp)[13] <- "soybean"
names(dfSuitTemp)[16] <- "summerwheat"
names(dfSuitTemp)[17] <- "winterwheat"

write.xlsx(dfSuitTemp,"datasets/climateResponse/dfSuitabilityTemperature_Mar2021.xlsx",row.names = F)
...
dfSuitPrec <- dfSuitPrec[,-(seq(3,33,2))]
names(dfSuitPrec)[13] <- "soybean"
names(dfSuitPrec)[16] <- "summerwheat"
names(dfSuitPrec)[17] <- "winterwheat"

write.xlsx(dfSuitPrec,"datasets/climateResponse/dfSuitabilityPrecipitation_Mar2021.xlsx",row.names = F)
