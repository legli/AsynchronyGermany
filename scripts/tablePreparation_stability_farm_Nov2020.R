...
length(unique(dfProduction$Item)) ## 10 crops
length(unique(dfProduction$Level)) ## 5133 farmers
...
## areas of crops in recent time interval
dfArea <- aggregate(AreaHarvested~Item+Year,dfProduction[which(dfProduction$Year%in%2008:2017),],sum)
dfArea <- aggregate(AreaHarvested~Item,dfArea,mean)
dfArea[order(dfArea$AreaHarvested,decreasing = T),"Item"]
...
lsAll <- lapply(vecDistrictFinal,function(lev){
  ...
  lsAggregate <- lapply(c(1998,2008),function(yearStart){
    ...
    lsFarm <- lapply(unique(dfProductionLevel$Level),function(f){
      ...
      data.frame(Level=f,nCrops=length(unique(dfProductionFarm$Item)),asynchronyB = 1-round(synchrony(dfProductionCropDet,time.var="Year",species.var="Item",abundance.var="Production"),10),areaHarvested=sum(dfProductionFarm$AreaHarvested))
    })
    dfFarm <- do.call(rbind,lsFarm)
    minCropsFarm = min(dfFarm$nCrops)
    maxCropsFarm = max(dfFarm$nCrops)
    asynchronyBfarm = weighted.mean(dfFarm$asynchronyB,dfFarm$areaHarvested) 
    ...
    lsAsynchronyCrops <- lapply(unique(dfProductionLevel$Item),function(i){
      ...
      data.frame(Item=i,AreaHarvested=sum(dfCrop$AreaHarvested),asynchronyCrop= 1-synchrony(dfCropDet,time.var="Year",species.var="Level",abundance.var="Production"))
    ...
    data.frame(district=lev,timePeriod=yearStart,nFarms=length(unique(dfProductionLevel$Level)),nCrops=length(unique(dfProductionLevel$Item)),nCropsFarmMin=minCropsFarm,nCropsFarmMax=maxCropsFarm,stabilityProduction=stability,asynchronyBetween_region=asynchronyB,asynchronyBetween_farm=asynchronyBfarm,asynchronyWithin=asynchronyW,areaHarvested=sum(dfYieldLevel$AreaHarvested),diversity=mean(dfShannonLevel$diversity))
...
write.csv(dfAll, "datasetsDerived/dataFinal_all_farm_Nov2020.csv",row.names=F)
...
write.csv(dfAll, "datasetsDerived/dataFinal_sampling_farm_Nov2020.csv",row.names=F)


#### summarize relevant metrics per time period at farm level
vecFarms <- Reduce(intersect,list(dfProduction$Level,dfYield$Level))

lsAll <- lapply(vecFarms,function(lev){
  # total production
  show(as.character(lev))
  lsAggregate <- lapply(c(1998,2008),function(yearStart){
    dfProductionLevel <- dfProduction[which(dfProduction$Level==lev&dfProduction$Year>=yearStart&dfProduction$Year<=(yearStart+9)),]
    dfYieldLevel <- dfYield[which(dfYield$Level==lev&dfYield$Year>=yearStart&dfYield$Year<=(yearStart+9)),]

    if (nrow(dfYieldLevel)==10){
      ...
      dfProductionCropDet <- do.call(rbind,lsProductionCropDet)
      data.frame(farm=lev,district=unique(dfYieldLevel$REGION_ID),timePeriod=yearStart,nCrops=length(unique(dfProductionLevel$Item)),stabilityProduction=stability,asynchronyBetween = 1-round(synchrony(dfProductionCropDet,time.var="Year",species.var="Item",abundance.var="Production"),10),areaHarvested=sum(dfProductionLevel$AreaHarvested))
    }
  })
  do.call(rbind,lsAggregate)
})
...
head(dfAll)
sum(is.na(dfAll))
sum(dfAll$asynchronyB==0)
# dfAll <- dfAll[which(dfAll$asynchronyB>0),]
cor(dfAll$asynchronyB,dfAll$stabilityProduction)

## regression
library(nlme)
library(MuMIn)
modFarmLevel <- lme(stabilityProduction~asynchronyBetween,
               random=~1|district,method = "ML",
               data=dfAll)
r.squaredGLMM(modFarmLevel)
AIC(modFarmLevel)
summary(modFarmLevel)

nrow(unique(dfAll[,c("farm","timePeriod")])) == nrow(dfAll) # check duplicates
length(unique(dfAll$farm))
